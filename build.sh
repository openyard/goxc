...
go build -installsuffix netgo -tags netgo -a -ldflags "-s -X github.com/openyard/goxc.version=${CI_BUILD_REF_NAME}-${BUILD_NUMBER} -X github.com/openyard/goxc.rev=${CI_BUILD_REF} -X github.com/openyard/goxc.built=${BUILD_STAMP}" -o goxc main/goxc.go
